...
#define DL_OPT_SUBDEV_RATE_MIN_TX	BIT(37)
#define DL_OPT_SUBDEV_RATE_MAX_TX	BIT(38)
#define DL_OPT_SUBDEV_RATE_GROUP_NAME	BIT(39)
#define DL_OPT_SUBDEV_RATE_PARENT	BIT(40)
...
struct dl_opts {
	...
	uint32_t subdev_rate_min_tx;
	uint32_t subdev_rate_max_tx;
...
static const enum mnl_attr_data_type devlink_policy[DEVLINK_ATTR_MAX + 1] = {
	...
	[DEVLINK_ATTR_SUBDEV_RATE_MIN_TX] = MNL_TYPE_U32,
	[DEVLINK_ATTR_SUBDEV_RATE_MAX_TX] = MNL_TYPE_U32,
	[DEVLINK_ATTR_SUBDEV_RATE_GROUP_NAME] = MNL_TYPE_NUL_STRING,
	[DEVLINK_ATTR_SUBDEV_RATE_PARENT] = MNL_TYPE_NUL_STRING,
};
...
static int __dl_argv_handle_subdev_rate_group(char *str, char **p_bus_name,
					      char **p_dev_name,
					      char **p_group_name)
{
	char *handlestr;
	...
	if (err) {
		...
		pr_err("subdev rate group name must start with alphabetical character\n");
		return -EINVAL;
	}
...
static int dl_argv_handle_subdev_rate_group(struct dl *dl, char **p_bus_name,
					    char **p_dev_name,
					    char **p_group_name)
{
	char *str = dl_argv_next(dl);
	...
	slash_count = strslashcount(str);
	if (slash_count == 2) {
		return __dl_argv_handle_subdev_rate_group(str, p_bus_name,
							  p_dev_name,
							  p_group_name);
	} else {
				...
				enum devlink_subdev_rate_type *p_type)
{
	if (strcmp(typestr, "leaf") == 0) {
		...
		*p_type = DEVLINK_SUBDEV_RATE_NODE;
	} else {
			 ...
			 uint64_t o_optional)
{
	...
	} else if (o_required & DL_OPT_SUBDEV_RATE_GROUP_NAME) {
		err = dl_argv_handle_subdev_rate_group(dl, &opts->bus_name,
						       &opts->dev_name,
						       &opts->rate_group_name);
		if (err)
			return err;
		o_found |= DL_OPT_SUBDEV_RATE_GROUP_NAME;
	}
	...
	while (dl_argc(dl)) {
		if (dl_argv_match(dl, "type") &&
			...
			o_found |= DL_OPT_SUBDEV_RATE_TYPE;
			   ...
			   (o_all & DL_OPT_SUBDEV_RATE_MIN_TX)) {
			dl_arg_inc(dl);
			err = dl_argv_uint32_t(dl, &opts->subdev_rate_min_tx);
			if (err)
				return err;
			o_found |= DL_OPT_SUBDEV_RATE_MIN_TX;
		} else if (dl_argv_match(dl, "max_tx_rate") &&
			   (o_all & DL_OPT_SUBDEV_RATE_MAX_TX)) {
			dl_arg_inc(dl);
			err = dl_argv_uint32_t(dl, &opts->subdev_rate_max_tx);
			if (err)
				return err;
			o_found |= DL_OPT_SUBDEV_RATE_MAX_TX;
		} else if (dl_argv_match(dl, "parent") &&
			   (o_all & DL_OPT_SUBDEV_RATE_PARENT)) {
			dl_arg_inc(dl);
			err = dl_argv_str(dl, &opts->rate_parent);
			if (err)
				return err;
			o_found |= DL_OPT_SUBDEV_RATE_PARENT;
...
static void dl_opts_put(struct nlmsghdr *nlh, struct dl *dl)
{
	...
	if (opts->present & DL_OPT_HANDLE) {
		...
		mnl_attr_put_u32(nlh, DEVLINK_ATTR_SUBDEV_INDEX,
				 opts->subdev_index);
	} else if (opts->present & DL_OPT_SUBDEV_RATE_GROUP_NAME) {
		mnl_attr_put_strz(nlh, DEVLINK_ATTR_BUS_NAME, opts->bus_name);
		mnl_attr_put_strz(nlh, DEVLINK_ATTR_DEV_NAME, opts->dev_name);
		mnl_attr_put_strz(nlh, DEVLINK_ATTR_SUBDEV_RATE_GROUP_NAME,
				  opts->rate_group_name);
	}
	...
	if (opts->present & DL_OPT_SUBDEV_RATE_TYPE)
		mnl_attr_put_u16(nlh, DEVLINK_ATTR_SUBDEV_RATE_TYPE,
				 opts->subdev_rate_type);
	if (opts->present & DL_OPT_SUBDEV_RATE_MIN_TX)
		mnl_attr_put_u32(nlh, DEVLINK_ATTR_SUBDEV_RATE_MIN_TX,
				 opts->subdev_rate_min_tx);
	if (opts->present & DL_OPT_SUBDEV_RATE_MAX_TX)
		mnl_attr_put_u32(nlh, DEVLINK_ATTR_SUBDEV_RATE_MAX_TX,
				 opts->subdev_rate_max_tx);
	if (opts->present & DL_OPT_SUBDEV_RATE_PARENT)
		mnl_attr_put_strz(nlh, DEVLINK_ATTR_SUBDEV_RATE_PARENT,
				  opts->rate_parent);
...
static void cmd_subdev_help(void)
{
	...
	pr_err("       devlink subdev rate del DEV/GROUP_NAME\n");
...
static void pr_out_subdev_rate_handle_start(struct dl *dl, struct nlattr **tb,
					    bool try_nice, uint16_t type)
{
	...
	if (type == DEVLINK_SUBDEV_RATE_LEAF) {
		...
		subdev_index = mnl_attr_get_u32(tb[DEVLINK_ATTR_SUBDEV_INDEX]);
		__pr_out_port_handle_start(dl, bus_name, dev_name, subdev_index,
					   try_nice, false);
	} else if (type == DEVLINK_SUBDEV_RATE_NODE) {
		...
		name_attr = tb[DEVLINK_ATTR_SUBDEV_RATE_GROUP_NAME];
...
static void pr_out_subdev_rate_handle_end(struct dl *dl)
{
	pr_out_port_handle_end(dl);
}
...
static void pr_out_subdev_rate(struct dl *dl, struct nlattr **tb,
			       uint16_t type)
{
	struct nlattr *min_tx_rate_attr = tb[DEVLINK_ATTR_SUBDEV_RATE_MIN_TX];
	struct nlattr *max_tx_rate_attr = tb[DEVLINK_ATTR_SUBDEV_RATE_MAX_TX];
	struct nlattr *parent_attr = tb[DEVLINK_ATTR_SUBDEV_RATE_PARENT];

	pr_out_subdev_rate_handle_start(dl, tb, false, type);
	...
	pr_out_subdev_rate_handle_end(dl);
}
...
static int cmd_subdev_rate_show_cb(const struct nlmsghdr *nlh, void *data)
{
	...
	type = mnl_attr_get_u16(tb[DEVLINK_ATTR_SUBDEV_RATE_TYPE]);
	if ((type == DEVLINK_SUBDEV_RATE_LEAF &&
	     !tb[DEVLINK_ATTR_SUBDEV_INDEX]) ||
	    (type == DEVLINK_SUBDEV_RATE_NODE &&
	     !tb[DEVLINK_ATTR_SUBDEV_RATE_GROUP_NAME]))
		return MNL_CB_ERROR;

	pr_out_subdev_rate(dl, tb, type);
	return MNL_CB_OK;
}
...
static int cmd_subdev_rate_set(struct dl *dl)
{
	struct nlmsghdr *nlh;
	int err;

	nlh = mnlg_msg_prepare(dl->nlg, DEVLINK_CMD_SUBDEV_RATE_SET,
			       NLM_F_REQUEST | NLM_F_ACK);
...
static int cmd_subdev_rate_add(struct dl *dl)
{
	struct nlmsghdr *nlh;
	int err;

	nlh = mnlg_msg_prepare(dl->nlg, DEVLINK_CMD_SUBDEV_RATE_NEW,
			       NLM_F_REQUEST | NLM_F_ACK);
...
static int cmd_subdev_rate_del(struct dl *dl)
{
	struct nlmsghdr *nlh;
	int err;

	nlh = mnlg_msg_prepare(dl->nlg, DEVLINK_CMD_SUBDEV_RATE_DEL,
			       NLM_F_REQUEST | NLM_F_ACK);
...
static int cmd_subdev_rate(struct dl *dl)
{
	if (dl_argv_match(dl, "show") || dl_no_arg(dl)) {
		...
		dl_arg_inc(dl);
		return cmd_subdev_rate_set(dl);
	} else if (dl_argv_match(dl, "add")) {
		dl_arg_inc(dl);
		return cmd_subdev_rate_add(dl);
	} else if (dl_argv_match(dl, "del")) {
		dl_arg_inc(dl);
		return cmd_subdev_rate_del(dl);
	}
...
static int cmd_mon_show_cb(const struct nlmsghdr *nlh, void *data)
{
	...
	case DEVLINK_CMD_SUBDEV_DEL:
		...
		pr_out_subdev(dl, tb);
		break;
	case DEVLINK_CMD_SUBDEV_RATE_GET: /* fall through */
	case DEVLINK_CMD_SUBDEV_RATE_SET: /* fall through */
	case DEVLINK_CMD_SUBDEV_RATE_NEW: /* fall through */
	case DEVLINK_CMD_SUBDEV_RATE_DEL: {
		uint16_t type;
