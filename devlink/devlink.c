...
static const enum mnl_attr_data_type devlink_policy[DEVLINK_ATTR_MAX + 1] = {
	...
	[DEVLINK_ATTR_SLICE_INDEX] = MNL_TYPE_U32,
	[DEVLINK_ATTR_SLICE_FLAVOUR] = MNL_TYPE_U16,
	[DEVLINK_ATTR_SLICE_PF_INDEX] = MNL_TYPE_U32,
...
static int __dl_argv_handle_slice(char *str, char **p_bus_name,
				 char **p_dev_name, uint32_t *p_slice_index)
{
	...
	err = strtouint32_t(portstr, p_slice_index);
	if (err) {
...
static int dl_argv_handle_slice(struct dl *dl, char **p_bus_name,
			       char **p_dev_name, uint32_t *p_slice_index)
{
	char *str = dl_argv_next(dl);
	unsigned int slash_count;
	...
	if (!str) {
		pr_err("Slice identification (\"bus_name/dev_name/slice_index\" .\n");
		return -EINVAL;
	}
	slash_count = strslashcount(str);
	if (slash_count == 2)
		return __dl_argv_handle_slice(str, p_bus_name,
					      p_dev_name, p_slice_index);
	else {
		pr_err("Wrong slice identification string format.\n");
		pr_err("Expected \"bus_name/dev_name/slice_index\".\n");
		return -EINVAL;
	}
}

static int __dl_argv_handle_slice_rate_group(char *str, char **p_bus_name,
					     char **p_dev_name,
					     char **p_group_name)
{
	char *handlestr;
	...
	err = strslashrsplit(str, &handlestr, p_group_name);
	if (err) {
		pr_err("slice identification \"%s\" is invalid\n", str);
		return err;
	} else if (!isalpha(**p_group_name)) {
		pr_err("slice rate group name must start with alphabetical character\n");
		return -EINVAL;
	}
...
static int dl_argv_handle_slice_rate_group(struct dl *dl, char **p_bus_name,
					   char **p_dev_name,
					   char **p_group_name)
{
	char *str = dl_argv_next(dl);
	...
	if (!str) {
		pr_err("Rate group identification (\"bus_name/dev_name/group_name\" .\n");
		return -EINVAL;
	}
	slash_count = strslashcount(str);
	if (slash_count == 2) {
		return __dl_argv_handle_slice_rate_group(str, p_bus_name,
							 p_dev_name,
							 p_group_name);
	} else {
		pr_err("Wrong rate group identification string format.\n");
		pr_err("Expected \"bus_name/dev_name/group_name\".\n");
		return -EINVAL;
	}
}
...
static int slice_rate_type_get(const char *typestr,
			       enum devlink_slice_rate_type *p_type)
{
	if (strcmp(typestr, "leaf") == 0) {
		*p_type = DEVLINK_SLICE_RATE_LEAF;
	} else if (strcmp(typestr, "node") == 0) {
		*p_type = DEVLINK_SLICE_RATE_NODE;
	} else {
		pr_err("Unknown slice rate type \"%s\"\n", typestr);
		return -EINVAL;
	}
	return 0;
}
			 ...
			 uint64_t o_optional)
{
	...
	} else if (o_required & DL_OPT_SLICE_RATE_GROUP_NAME) {
		err = dl_argv_handle_slice_rate_group(dl, &opts->bus_name,
						      &opts->dev_name,
						      &opts->rate_group_name);
		if (err)
			return err;
		o_found |= DL_OPT_SLICE_RATE_GROUP_NAME;
	}
	...
	while (dl_argc(dl)) {
		if (dl_argv_match(dl, "type") &&
			...
			if (opts->hw_addr_len < 0)
				return -EINVAL;
			o_found |= DL_OPT_SLICE_HW_ADDR;
		} else if (dl_argv_match(dl, "type") &&
			   (o_all & DL_OPT_SLICE_RATE_TYPE)) {
			enum devlink_slice_rate_type type;
			const char *typestr;
...
static void dl_opts_put(struct nlmsghdr *nlh, struct dl *dl)
{
	...
	if (opts->present & DL_OPT_HANDLE) {
		...
		mnl_attr_put_strz(nlh, DEVLINK_ATTR_REGION_NAME,
				  opts->region_name);
	} else if (opts->present & DL_OPT_HANDLE_SLICE) {
		mnl_attr_put_strz(nlh, DEVLINK_ATTR_BUS_NAME, opts->bus_name);
		mnl_attr_put_strz(nlh, DEVLINK_ATTR_DEV_NAME, opts->dev_name);
		mnl_attr_put_u32(nlh, DEVLINK_ATTR_SLICE_INDEX,
				 opts->slice_index);
	} else if (opts->present & DL_OPT_SLICE_RATE_GROUP_NAME) {
		mnl_attr_put_strz(nlh, DEVLINK_ATTR_BUS_NAME, opts->bus_name);
		mnl_attr_put_strz(nlh, DEVLINK_ATTR_DEV_NAME, opts->dev_name);
		mnl_attr_put_strz(nlh, DEVLINK_ATTR_SLICE_RATE_GROUP_NAME,
				  opts->rate_group_name);
	}
	...
	if (opts->present & DL_OPT_NETNS)
		mnl_attr_put_u32(nlh,
				 ...
				 opts->netns);
	if (opts->present & DL_OPT_SLICE_HW_ADDR)
		mnl_attr_put(nlh, DEVLINK_ATTR_SLICE_HW_ADDR,
...
static bool dl_dump_filter(struct dl *dl, struct nlattr **tb)
{
	...
	struct nlattr *attr_slice_index = tb[DEVLINK_ATTR_SLICE_INDEX];
	...
	if (opts->present & DL_OPT_HANDLE_SLICE &&
	    attr_bus_name && attr_dev_name && attr_port_index) {
		uint32_t slice_index = mnl_attr_get_u32(attr_slice_index);
		const char *bus_name = mnl_attr_get_str(attr_bus_name);
		const char *dev_name = mnl_attr_get_str(attr_dev_name);

		if (strcmp(bus_name, opts->bus_name) != 0 ||
		    strcmp(dev_name, opts->dev_name) != 0 ||
		    slice_index != opts->slice_index)
			return false;
	}
	return true;
}
...
static void pr_out_slice_handle_start(struct dl *dl, struct nlattr **tb,
				     bool try_nice)
{
	uint32_t slice_index = mnl_attr_get_u32(tb[DEVLINK_ATTR_SLICE_INDEX]);
	const char *bus_name = mnl_attr_get_str(tb[DEVLINK_ATTR_BUS_NAME]);
	const char *dev_name = mnl_attr_get_str(tb[DEVLINK_ATTR_DEV_NAME]);
...
static void cmd_slice_help(void)
{
...
static void pr_out_slice(struct dl *dl, struct nlattr **tb)
{
	struct nlattr *port_index_attr = tb[DEVLINK_ATTR_PORT_INDEX];
	struct nlattr *hw_addr_attr = tb[DEVLINK_ATTR_SLICE_HW_ADDR];
	struct nlattr *flavour_attr = tb[DEVLINK_ATTR_SLICE_FLAVOUR];
	struct nlattr *pf_attr = tb[DEVLINK_ATTR_SLICE_PF_INDEX];
	struct nlattr *vf_attr = tb[DEVLINK_ATTR_SLICE_VF_INDEX];

	pr_out_slice_handle_start(dl, tb, false);
	if (flavour_attr) {
		uint16_t flavour = mnl_attr_get_u16(flavour_attr);

		pr_out_str(dl, "flavour", slice_flavour_name(flavour));
		switch (flavour) {
		case DEVLINK_SLICE_FLAVOUR_PCI_PF:
		case DEVLINK_SLICE_FLAVOUR_PCI_VF:
			if (pf_attr) {
				...
				pr_out_uint(dl, "pfnum", pf_index);
				if (vf_attr) {
					uint32_t vf_index;

					vf_index = mnl_attr_get_u32(vf_attr);
					pr_out_uint(dl, "vfnum", vf_index);
				}
			}
			if (port_index_attr) {
				uint32_t port_index =
...
static int cmd_slice_show_cb(const struct nlmsghdr *nlh, void *data)
{
	struct genlmsghdr *genl = mnl_nlmsg_get_payload(nlh);
	struct nlattr *tb[DEVLINK_ATTR_MAX + 1] = {};
	struct dl *dl = data;
...
static int cmd_slice_show(struct dl *dl)
{
	uint16_t flags = NLM_F_REQUEST | NLM_F_ACK;
	struct nlmsghdr *nlh;
	int err;
	...
	nlh = mnlg_msg_prepare(dl->nlg, DEVLINK_CMD_SLICE_GET, flags);
	...
	pr_out_section_start(dl, "slice");
	err = _mnlg_socket_sndrcv(dl->nlg, nlh, cmd_slice_show_cb, dl);
	pr_out_section_end(dl);
	return err;
}

static int cmd_slice_set(struct dl *dl)
{
	struct nlmsghdr *nlh;
	int err;

	nlh = mnlg_msg_prepare(dl->nlg, DEVLINK_CMD_SLICE_SET,
			       NLM_F_REQUEST | NLM_F_ACK);

	err = dl_argv_parse_put(nlh, dl,
				DL_OPT_HANDLE_SLICE | DL_OPT_SLICE_HW_ADDR,
				0);
	if (err)
		return err;
...
static const char *slice_rate_type_name(uint16_t type)
{
	switch (type) {
	case DEVLINK_SLICE_RATE_LEAF:
		return "leaf";
	case DEVLINK_SLICE_RATE_NODE:
		return "node";
	default:
		return "<unknown type>";
	}
...
static void pr_out_rate_group_handle_start(struct dl *dl, const char *bus_name,
					   const char *dev_name,
					   const char *group_name)
{
	static char buf[128];

	snprintf(buf, sizeof(buf), "%s/%s/%s", bus_name, dev_name, group_name);
	if (dl->json_output) {
		jsonw_name(dl->jw, buf);
		jsonw_start_object(dl->jw);
	} else {
					   ...
					   bool try_nice, uint16_t type)
{
	...
	if (type == DEVLINK_SLICE_RATE_LEAF) {
		...
		name_attr = tb[DEVLINK_ATTR_SLICE_RATE_GROUP_NAME];
		group_name = mnl_attr_get_str(name_attr);
		pr_out_rate_group_handle_start(dl, bus_name, dev_name,
					       group_name);
	}
}
...
static int cmd_slice_rate_show_cb(const struct nlmsghdr *nlh, void *data)
{
	...
	if ((type == DEVLINK_SLICE_RATE_LEAF &&
	     !tb[DEVLINK_ATTR_SLICE_INDEX]) ||
	    (type == DEVLINK_SLICE_RATE_NODE &&
	     !tb[DEVLINK_ATTR_SLICE_RATE_GROUP_NAME]))
		return MNL_CB_ERROR;
...
static int cmd_slice_rate_show(struct dl *dl)
{
	uint16_t flags = NLM_F_REQUEST | NLM_F_ACK;
	struct nlmsghdr *nlh;
	int err;
	...
	pr_out_section_start(dl, "slice rate");
	err = _mnlg_socket_sndrcv(dl->nlg, nlh, cmd_slice_rate_show_cb, dl);
	pr_out_section_end(dl);
	return err;
}
...
static int cmd_slice(struct dl *dl)
{
	if (dl_argv_match(dl, "help")) {
		cmd_slice_help();
		return 0;
		   ...
		   dl_argv_match(dl, "list") || dl_no_arg(dl)) {
		dl_arg_inc(dl);
		return cmd_slice_show(dl);
	} else if (dl_argv_match(dl, "set")) {
		dl_arg_inc(dl);
		return cmd_slice_set(dl);
	} else if (dl_argv_match(dl, "rate")) {
		dl_arg_inc(dl);
		return cmd_slice_rate(dl);
	}
	pr_err("Command \"%s\" not found\n", dl_argv(dl));
	return -ENOENT;
}
...
static void pr_out_port(struct dl *dl, struct nlattr **tb)
{
	...
	if (tb[DEVLINK_ATTR_SLICE_INDEX])
		pr_out_uint(dl, "slice_index",
			    mnl_attr_get_u32(tb[DEVLINK_ATTR_SLICE_INDEX]));
	pr_out_port_handle_end(dl);
}
...
static const char *cmd_name(uint8_t cmd)
{
	...
	case DEVLINK_CMD_SLICE_RATE_NEW: return "add";
...
static void help(void)
{
	pr_err("Usage: devlink [ OPTIONS ] OBJECT { COMMAND | help }\n"
	       ...
	       "where  OBJECT := { dev | slice | port | sb | monitor | dpipe | resource | region | health | trap }\n"
...
static int dl_cmd(struct dl *dl, int argc, char **argv)
{
	...
	if (dl_argv_match(dl, "help") || dl_no_arg(dl)) {
		...
		dl_arg_inc(dl);
		return cmd_dev(dl);
	} else if (dl_argv_match(dl, "slice")) {
		dl_arg_inc(dl);
		return cmd_slice(dl);
	} else if (dl_argv_match(dl, "port")) {
		dl_arg_inc(dl);
